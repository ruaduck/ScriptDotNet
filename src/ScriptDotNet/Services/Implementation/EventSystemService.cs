    ...
    public class EventSystemService : BaseService, IEventSystemService
    {
        ...
        public event EventHandler<MessangerIncomingTextEventArgs> MessangerIncomingText
        {
            add
            {
                ...
                if (handler == null)
                    _client.SendPacket(PacketType.SCSetEventProc, EventTypes.MessengerEvent, _eventFunctionCounter++);
        ...
        public event EventHandler<SetGlobalVarEventArgs> SetGlobalVar
        {
            add
            {
                ...
                if (handler == null)
                    _client.SendPacket(PacketType.SCSetEventProc, EventTypes.SetGlobalVar, _eventFunctionCounter++);
                _setGlobalVar += value;
            }
            remove
            {
        ...
        public event EventHandler<UpdateObjectStatsEventArgs> UpdateObjectStats
        {
            add
            {
                ...
                if (handler == null)
                    _client.SendPacket(PacketType.SCSetEventProc, EventTypes.UpdateObjStats, _eventFunctionCounter++);
        ...
        private void ProcessEvent(ExecEventProcData data)
        {
            ...
            switch (data.EventType)
            {
                ...
                case EventTypes.ClilocSpeech:
                    OnClilocSpeech((int)data.Parameters[0], (string)data.Parameters[1], (string)data.Parameters[2]); //OnClilocSpeech((uint)data.Parameters[0], (string)data.Parameters[1], (uint)data.Parameters[2], (string)data.Parameters[3]);
                    break;
                case EventTypes.ClilocSpeechAffix:
                    OnClilocSpeechAffix((int)data.Parameters[0], (string)data.Parameters[1], (string)data.Parameters[2], (string)data.Parameters[3]);//OnClilocSpeechAffix((uint)data.Parameters[0], (string)data.Parameters[1], (uint)data.Parameters[2], (string)data.Parameters[3], (string)data.Parameters[4]);
                    break;
        ...
        {
            _messangerIncomingText?.Invoke(this, new MessangerIncomingTextEventArgs(messangerType, senderNickname, senderId, chatId, eventMsg, eventCode));
        }
        ...
        {
            _clilocSpeechAffix?.Invoke(this, new ClilocSpeechAffixEventArgs(senderId, senderName, affix, text));
        }

        private void OnClilocSpeech(int senderId, string senderName, string text)
        {
            _clilocSpeech?.Invoke(this, new ClilocSpeechEventArgs(senderId, senderName, text));
        }
